...
const sortByIdThenName = (a: [string, AllTimeCourseInfo | undefined], b: [string, AllTimeCourseInfo | undefined]) => {
    if (!a[1] && !b[1]) {
        return 0;
    }
    if (a[0] === b[0]) {
...
const VirtualizedList: React.FC<VirtualizedListProps> = ({options, onSelectedOption, isLoading, selectedOptions}) => {
    ...
    const filteredOptions = useMemo(
        () => Object.entries(options ?? {})
            .filter(([id, course]) =>
                ...
                course?.name?.toLowerCase().includes(search.replace("-", "").toLowerCase())
            )
    ...
    useEffect(() => {
        if (focusedIndex !== null) {
            rowVirtualizer.scrollToIndex(focusedIndex, {
                align: "center",
                behavior: focusedIndex == filteredOptions.length - 1 || focusedIndex == 0 ? "auto" : "smooth",
            });
        }
                            ...
                            return (
                                <div
                                    ...
                                    className={cn(
                                        ...
                                        isSelected && isFocused && "bg-zinc-300/80 hover:bg-zinc-300/80"
                                    )}
                                    style={{
