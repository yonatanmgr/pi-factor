...
import { cn, getSemesterName } from "@/lib/utils/utils";
...
const TopLecturers: React.FC<TopLecturersProps> = ({ semesterDataResults, language }) => {

  const lecturerData = new Map<string, { 
    total: number; 
    count: number;
  ...
  semesterDataResults.forEach(({ semester, processedData }) => {
    processedData.lecturers.forEach(lecturer => {
      ...
      if (isNaN(processedData.averageMean) || processedData.averageMean === undefined) return;
  ...
  return (
    <div className="flex flex-row overflow-x-auto overflow-y-hidden items-center gap-2 py-2 px-4 bg-neutral-200/50 dark:bg-neutral-800/50 rounded-lg">
      <h3 className={cn("text-sm flex flex-row gap-1.5 items-center select-none min-w-fit text-neutral-600 dark:text-neutral-400", ibmPlexSansHebrew.className)}>
          ...
          {sortedLecturers.map(({ lecturer, average, semesters }, index) => (
            <Tooltip key={lecturer}>
              <TooltipTrigger asChild>
                ...
                >
                  <span className={cn(ibmPlexSansHebrew.className)}>{index + 1}. {lecturer}</span>
                  <span className="text-neutral-500 min-w-fit dark:text-neutral-400 font-mono">
                    {average.toFixed(2)}
                  </span>
                </div>
              </TooltipTrigger>
              <TooltipContent dir="rtl" className="max-w-[200px] overflow-hidden">
                <div className="text-xs flex flex-wrap gap-1">
                  {semesters.toReversed().map((s, index) => <span key={index}>{getSemesterName(s, language)}{index !== semesters.length - 1 && ', '}</span>)}
                </div>
              </TooltipContent>
            </Tooltip>
          ))}
